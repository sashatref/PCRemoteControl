...
{
    return FindWindow(NULL, QString("AIMP2_RemoteInfo").toStdString().c_str());
}

HWND LA::getLAWindow()
{
    return FindWindow(QString("LightAlloyFront").toStdString().c_str(), NULL);
}
...
void System::pcHibernate()
{
    QProcess::startDetached("shutdown.exe", QStringList() << "/h" << "/f");
}
